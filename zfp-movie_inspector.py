...
def main(argv=None):
    ...
    movie_out = os.path.join("output", "movies", "zfp-movies.csv")
    ...
    directors = extract_filtered_series(
        source_trimmed, ["movie_title", "director_name", "director_facebook_likes"]
    )
    ...
    actor_1 = extract_filtered_series(
        source_trimmed, ["movie_title", "actor_1_name", "actor_1_facebook_likes"]
    )
    ...
    actor_2 = extract_filtered_series(
        source_trimmed, ["movie_title", "actor_2_name", "actor_2_facebook_likes"]
    )

    actor_2_out = os.path.join("output", "movies", "zfp-actor_2.csv")
    ...
    actor_3 = extract_filtered_series(
        source_trimmed, ["movie_title", "actor_3_name", "actor_3_facebook_likes"]
    )

    actor_3_out = os.path.join("output", "movies", "zfp-actor_3.csv")
    ...
    movie_genres_split = (
        movie_genres.genres.apply(pd.Series)
    ...
    keywords["plot_keywords"] = keywords["plot_keywords"].str.split(
        "|", n=-1, expand=False
    )
    keywords_split = (
    ...
    movie_keywords = extract_filtered_series(
        source_trimmed, ["movie_title", "plot_keywords"]
    )
    movie_keywords["plot_keywords"] = movie_keywords["plot_keywords"].str.split(
        "|", n=-1, expand=False
    )
    ...
    movie_keywords_out = os.path.join(
        "output", "movies", "zfp-movie_keywords-split.csv"
    )
    ...
    movie_country_out = os.path.join("output", "movies", "zfp-movie_country.csv")
    ...
    country_out = os.path.join("output", "movies", "zfp-country.csv")
    ...
    movie_language = extract_filtered_series(
        source_trimmed, ["movie_title", "language"]
    )

    movie_language_out = os.path.join("output", "movies", "zfp-movie_language.csv")
    write_series_to_csv(movie_language, movie_language_out, ",", False)
    logging.info(msg[13].format(os.path.abspath(movie_country_out)))
    # language table
    language = extract_filtered_series(source_trimmed, ["language"])
    language_out = os.path.join("output", "movies", "zfp-language.csv")
    write_series_to_csv(language, language_out, ",", False)
    logging.info(msg[14].format(os.path.abspath(language_out)))

    # movie_rating temp table
    movie_rating = extract_filtered_series(source_trimmed, ["movie_title", "content_rating"])

    movie_rating_out = os.path.join("output", "movies", "zfp-movie_rating.csv")
    write_series_to_csv(movie_rating, movie_rating_out, ",", False)
    logging.info(msg[15].format(os.path.abspath(movie_rating_out)))
    # rating table
    rating = extract_filtered_series(source_trimmed, ["content_rating"])
    rating_out = os.path.join("output", "movies", "zfp-rating.csv")
    write_series_to_csv(rating, rating_out, ",", False)
    logging.info(msg[16].format(os.path.abspath(rating_out)))
